# #' @title Empirical Bayes Poisson Mean (mixture of two gammas as prior, fast versdion 2)
# #' @description Uses Empirical Bayes to fit the model \deqn{x_j | \lambda_j ~ Poi(s_j \lambda_j)} with \deqn{lambda_j ~ g()}
# #' with Point Gamma: g()  = pi_0 gamma(shape1, scale1) + (1-pi_0) gamma(shape2, scale2). It is supposed to be faster than \eqn{ebpm_two_gamma}
# #' 
# #' @import stats
...
# #'    s = replicate(100,1)
# #'    out = ebpm_two_gamma_fast(x,s)
# #' @export

# ebpm_two_gamma_fast2 <- function(x, s = 1, g_init = NULL, fix_g = F,control = NULL){
#   if(length(s) == 1){s = replicate(length(x),s)}
#   if(is.null(control)){control = nlm_control_defaults()}
#   if(is.null(g_init)){g_init = init_two_gamma_fast2(x, s); fix_g =  F}
#   g_init = list(pi0 = g_init$pi0, shape = 0.5*( g_init$shape1 +  g_init$shape2), 
#                 scale1 = g_init$scale1, scale2 = g_init$scale2)
#   #browser()
#   if(!fix_g){
#     ## MLE
#     fn_params = list(x = x, s = s)
#     opt = do.call(nlm, c(list(tg_nlm_fn2, tg_transform_param2(g_init)), fn_params, control))
#     log_likelihood =  -tg_nlm_fn2(opt$estimate, x, s)
#     opt_g = tg_transform_param_back2(opt$estimate)
#   }else{ ## fix_g = T
...
#     log_likelihood =  -tg_nlm_fn2(tg_transform_param2(opt_g), x, s)
#   }
  
#   fitted_g = list(pi0 = opt_g[1], shape1 = opt_g[2], scale1  = opt_g[3], shape2 = opt_g[2], scale2  = opt_g[4])
...
#   b1  =  exp(par[3])
#   b2  =  exp(par[4])
#   d1_log <- dnbinom_cts_log_vec(x, a, b1/(b1+s))
#   d2_log <- dnbinom_cts_log_vec(x, a, b2/(b2+s))
...
# tg_transform_param2 <- function(par0){
...
# tg_transform_param_back2 <- function(par){
...
# init_two_gamma_fast2 <- function(x, s){
#   #browser()
