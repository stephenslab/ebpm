...
#' @export
ebpm_two_gamma_fast5 <- function(x, s = 1, g_init = NULL, 
...
loglik.tg <- function(x, s, g){
  ...
  a2 = g$shape2
  b2 = 1/g$scale2
  loglik = sum(log(pi1 * exp(loglik.nb(x, s, a1, b1)) + pi2 * exp(loglik.nb(x, s, a2, b2))))
  return(loglik)
}
...
lam.posterior <- function(x, s, g){
  ...
  pi1_ = pi0 * exp(loglik.nb(x, s, a1, b1))
  pi1 = pi1_/(pi1_ + (1-pi0) * exp(loglik.nb(x, s, a2, b2)))
  pi2 = 1 - pi1
