...
source("../ebpm_two_gamma_fast4.R")
source("../ebpm_point_gamma.R")
...
start = proc.time()
fit_ebpmf_tg = ebpmf.alpha::ebpmf(X, K = k,pm_func = ebpm_two_gamma_fast4,
                                  pm_control = list(n_iter = 10),
                                  init = init, maxiter = maxiter, verbose = verbose, tol = tol)
runtime = proc.time()- start
fit_ebpmf_tg$runtime = runtime[[3]]

saveRDS(list(fit = fit_ebpmf_tg, sessionInfo = sessionInfo()), "test_ebpmf_two_gamma_fast4.Rds")