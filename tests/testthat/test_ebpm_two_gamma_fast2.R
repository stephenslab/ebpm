...
rm(list = ls())
devtools::load_all(".")
context("test_ebpm_two_gamma_fast2")
library(ebpm)
set.seed(123)
...
g_true = two_gamma(pi0 = 0.6, shape1 = 20, scale1 = 1/10, shape2 = 20, scale2 = 1)
sim = simulate_two_gamma_poisson(g_true, s = 1, n_sample = 1000)
  
fit_start_truth = ebpm::ebpm_two_gamma_fast2(sim$x, sim$s, g_init = g_true)

fit <- ebpm::ebpm_two_gamma_fast2(sim$x, sim$s)


fit_fixg <- ebpm::ebpm_two_gamma_fast2(sim$x, sim$s, g_init = fit$fitted_g, fix_g = T)
